...
describe('checks that wait for the event', () => {
  ...
  it('scores a future event only on what can be answered today', async () => {
    ...
    // Not 100%: community credits are still counted, because a drupal.org event
    // page lists organisers BEFORE the event — answerable now, and the fixture
    // has none. Provenance counts too: the fixture's sponsor has no listing page
    // recorded and its records cite nothing. Only the three post-event checks
    // are excluded.
    expect(find(cov, 'future.json').score).toBe(70);
  });
