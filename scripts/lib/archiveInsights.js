...
export async function buildInsights(dataDir, store) {
  ...
  // An event's SERIES is what it belongs to, which is not always what it was
  // called. Two DrupalSouth editions were marketed as Drupal Down Under; DrupalGov
  // 2020 ran on drupalsouth.org. The mapping is per event and lives in the
  // curation ledger, so datasets stay byte-identical — same contract as the
  // speaker/sponsor aliases above.
  ...
  for (const file of files) {
    ...
    const series = seriesFor(file, ev);
    ...
    for (const person of community) {
      ...
      rec.events.set(file, {
        ...
        // The album view orders by when the event happened, not by its name —
        // `year` alone cannot separate two events in the same year.
        startDate: str(ev.startDate) || null,
        lat: coords?.lat,
        lon: coords?.lon,
      });
    }
    ...
    const items = allItems.filter(countsAsSession);
    sessions += items.length;
    // The mix inside the programme: how much of it is hands-on, and how much of
    // the schedule around it is social. Workshops are part of `sessions`;
    // socials sit outside it, which is why they are counted separately.
    for (const i of items) if (itemKind(i) === 'workshop') workshops += 1;
    for (const i of allItems) if (itemKind(i) === 'social' && !i.cancelled) socials += 1;
    ...
    if (year)
      (yearEvents[year] ??= []).push({
        ...
        // `year` alone cannot separate two events in the same year.
        startDate: str(ev.startDate) || null,
        // Where this event's data came from, summarised for the Sources view.
        // Computed here rather than read from catalog.json because this loop
        // already holds the whole dataset — the per-record citations needed for
        // the exact/page split are in `items`, which the catalog does not carry.
        sources: provenanceSummary(data),
        lat: coords?.lat,
        lon: coords?.lon,
      });
